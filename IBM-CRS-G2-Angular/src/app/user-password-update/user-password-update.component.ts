...
export class UserPasswordUpdateComponent implements OnInit {
  ...
  onUpdate() {
    this.passwordDto=new PasswordDto(this.passwordUpdateForm.value["Email"], this.passwordUpdateForm.value["oldPassword"], this.passwordUpdateForm.value["newPassword"]);


    const headers={"Content-Type": "application/json"};
    const body=JSON.stringify(this.passwordDto);

    this.http.post<any>("http://localhost:1901/user/passwordchange", body, {headers}).subscribe(
